...
if (process.env.NODE_ENV !== 'production') {
  ...
  app.use(morgan("tiny"))
  const Gmail_temp=require("./models/index.js")
...
async function main() {
  const dbUrl = process.env.DB_URL || "mongodb://user:password@127.0.0.1:27017/gmail_t"; // Default URL if no environment variable is provided
  
  try {
    // Connect to the MongoDB database
    await mongoose.connect(dbUrl, {
      useNewUrlParser: true, // Parses the connection string properly
...
app.use((req,res,next)=>{
  ...
  app.get("/",(req,res)=>{
    res.redirect("/gmail_temp")
  })
app.get("/gmail_temp", catchAsync(async (req, res,next) => {
    const gmail_temp = await Gmail_temp.find(); // Ensure the connection is established before this query
  ...
  app.get("/gmail_temp/:id/download", catchAsync(async (req, res,next) => {
  ...
  app.post("/gmail_temp/", upload.array("image"),catchAsync(async(req,res,next)=>{
  ...
  app.all("*",(req,res,next)=>{
    next(new ExpressError("this is not good ",404));
 })
